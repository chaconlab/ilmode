...
#suff="_af1"  # general output suffix (bt "both terminals", nt "n-terminal", etc...)
#opts="-m 3 --skip_missingatoms -a 1 --ns 10000 --flanks 1 --aliflanks --drmsd 0.25"
...
opts="-m 3  --skip_missingatoms -a 1 --ns 10000 --drmsd 0.25"
#suff="_aF0"  # general output suffix
#opts="-m 2 --skip_missingatoms -a 1 --ns 10000 --drmsd 0.25"
#suff="_aF2"  # general output suffix (bt "both terminals", nt "n-terminal", etc...)
 ...
 do
  ...
  do
   ...
   then
    ...
    line=`grep "ilmode> Flanks RMSD" $log`
    rmsdA=`echo $line | awk '{print $7}'`
    rmsdA2=`echo $line | awk '{print $12}'`
    line=`grep "ilmode> Residues:" $log`
    nres=`echo $line | awk '{print $3}'`
    ndh=`echo $line | awk '{print $5}'`
    echo "$log2  $delta0 $rmsdA $rmsdA2 $rmsd0 $rmsdF $rmsd0NCAC $rmsdNCAC      $rmsdD $motion $nres $ndh" >> $stat  # dump statistics (append)
   fi
  done
 done
done

...
The first command moves forward (-a 1) the 81-93 loop until the rmsd from the initial position is > 3.0 Å (--rmsd 3.0). Every 0.25Å (--drmsd 0.25) away from the initial pose the moved loop coordinates are saved in the 3hsz_mod1F_traj.pdb trajectory file.  The second command does the same but backward, and the final generate a forward-backward trajectory like this: 
...
Note how the loop closure is fully maintained despite the large motion. Please, activate the loop option (right-click on the play icon) for a better display. 
